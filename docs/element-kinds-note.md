...
>
> **学習ノート枠の濫用防止 (新規追加基準)**: 本書は exceptional treatment であり repo の defaults ではない。「9× 未達なら `docs/<topic>-note.md` で保全」を標準動線として運用しない。`docs/<topic>-note.md` の新規追加は **(i)** pattern として 9× 未達 **かつ (ii)** 一次情報を verbatim 引用可能 ([docs/primary-sources.md](primary-sources.md) Tier 1 / Tier 2 該当) **かつ (iii)** 機構解説に独立した学習価値がある (本 repo の 3 カテゴリ判別フローに該当しない engine 内部表現の理解、など) 場合に限る。3 条件を満たさない 9× 未達 Issue は学習ノートではなく Issue コメント / decision log で記録する。
...
V8 公式 ([docs/primary-sources.md — Tier 1: V8 公式 `elements-kinds`](primary-sources.md#tier-1-v8-公式-primary)) からの verbatim 引用:
...
### `fill` 例外 (2025-02-28 時点)

`Array.prototype.fill` のみ HOLEY → PACKED 復帰経路が例外的に追加されている (引用日 2025-02-28、hub の [Tier 1 `elements-kinds`](primary-sources.md#tier-1-v8-公式-primary) 注記参照)。それ以外の埋め戻し (`for` ループ代入・`copyWithin` 等) はこの例外に含まれない。
...
`Array.prototype.fill` は HOLEY → PACKED 復帰経路として安全 (hub の [Tier 1 `elements-kinds`](primary-sources.md#tier-1-v8-公式-primary) 「`Array.prototype.fill` の例外（2025-02-28 時点）」を参照)。
...
引用 Tier は [docs/primary-sources.md](primary-sources.md) 体系に従う（Tier 1: 公式 / spec、Tier 2: engine team、Tier 3: 信頼性ある二次、Tier 4: 理論）。本書本文で直接引用した一次情報の所在は以下。

- **Tier 1 (V8 公式)**: [`elements-kinds`](primary-sources.md#tier-1-v8-公式-primary) — 2D lattice / 一方向遷移 verbatim 引用 2 件 / `Array.prototype.fill` 例外 (2025-02-28 時点)
- **Tier 1 (V8 公式)**: [`maglev`](primary-sources.md#tier-1-v8-公式-primary) — Maglev mid-tier の制約 (escape analysis を持たない)
- **Tier 1 (V8 公式)**: [`fast-properties`](primary-sources.md#tier-1-v8-公式-primary) — hidden class / dictionary mode 退化条件
- **Tier 1 (V8 公式)**: [`jitless`](primary-sources.md#tier-1-v8-公式-primary) — `--jitless` モード (Ignition のみ動作、Phase B `--jitless` 実測の根拠)
- **Tier 2 (V8 team member)**: [Bynens — `shapes-ics`](primary-sources.md#tier-2-v8-team-member--著名-perf-エンジニア-primary-相当) / [Bynens — `array-fill-undefined`](primary-sources.md#tier-2-v8-team-member--著名-perf-エンジニア-primary-相当) — hole vs `undefined` の境界
- pattern 化を defer-needs-rebench とした検証結果は Issue [#10](https://github.com/824ysuk/benchmark-antipatterns/issues/10) のコメントを参照

各 URL の Tier 区分・主題 → 根拠 URL 早見表・引用規律 (主題一致確認 / 条件併記 / 撤回履歴) は hub を参照してください。
